...
/** Every seeded Actor run is capped at this charge (Apify maxTotalChargeUsd) so a bad query cannot burn the free-tier credit. */
const DEFAULT_RUN_COST_CAP_USD = 0.1;
...
export const POST = withApi(
  async (ctx) => {
    ...
    if (missing.length) {
      await ctx.db.providerConfig.createMany({ data: missing.map((r) => ({ organizationId: null, domain: r.domain, provider: "apify", platform: r.platform, actorId: r.actorId, adapter: r.adapter, enabled: true, priority: r.priority, timeoutSec: 600, costLimitUsd: DEFAULT_RUN_COST_CAP_USD, settings: {} })) });
