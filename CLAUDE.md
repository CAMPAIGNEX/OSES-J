...
- Apify is on the free tier: never start an Actor run (searches, enrichment, content, tests that execute Actors) without the user's explicit permission; metadata calls (`/users/me`, `getActor`, dataset reads) are fine. Prefer the Google Custom Search provider for the search-engine strategy.
- Platform-wide keys live in the `platform_settings` row, edited at OS-Panel → Providers & keys (`/api/os-panel/platform`). Read them through `getPlatformConfig(db)` (`@oses/database`), never `getEnv()` directly: resolution is workspace override → platform row → env fallback. `?scope=platform` on the providers routes targets the platform Actor rows.
